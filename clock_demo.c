...
void ClockDemo_1sUpdate(BR_GFX_Canvas canvas)
{
	...
	// Draw the hour hand
	BR_GFX_DrawLine(63, 63, (((int16_t)BlackRam_GetTrig(Hours * (360/24), BLACK_RAM_COS) * HOUR_LENGTH) / 100) + 63, (((int16_t)BlackRam_GetTrig(Hours * (360/24), BLACK_RAM_SIN) * HOUR_LENGTH) / 100) + 63, Color565(255,0,0), canvas);
	
	// Draw the minute hand
	BR_GFX_DrawLine(63, 63, (((int16_t)BlackRam_GetTrig(Minutes * (360/60), BLACK_RAM_COS) * MINUTE_LENGTH) / 100) + 63, (((int16_t)BlackRam_GetTrig(Minutes * (360/60), BLACK_RAM_SIN) * MINUTE_LENGTH) / 100) + 63, Color565(0,255,0), canvas);
